...
## 회고
처음 이 프로젝트를 시작했을 때 모노레포 구조와 NestJS 기반 아키텍처는 이미 잡혀 있었고, 단순히 기능을 구현하는 것보다 이 구조를 얼마나 제대로 이해하고 활용할 수 있는가가 더 큰 과제였다. 특히 pnpm workspace로 패키지가 분리되어 있고, 도메인 단위로 코드가 나뉘어 있는 상태였기 때문에, 처음에는 코드 한 줄을 수정하는 것조차 여기에서 수정하는 게 맞는지부터 고민해야 했다. </br>

초반에는 구조를 깊게 이해하지 못한 상태에서 구현을 진행하다 보니, 자연스럽게 중복 코드가 생기거나 다른 레이어의 책임을 침범하는 경우가 있었다. 기능은 동작했지만, 기존 구조와 맞지 않으면서 점점 코드가 어긋나는 느낌이 들었고, 그때부터 단순히 구현을 이어가는 게 아니라 구조가 왜 그렇게 나뉘어 있는가를 역으로 분석하기 시작했다. </br>

특히 인상깊었던 건, 도메인과 레이어가 나뉘어 있는 이유를 체감하게 된 순간이었다. 처음에는 컨트롤러, 서비스, 레포지토리 구조가 단순한 패턴이라고 생각했지만, 실제로 작업을 하면서 각각의 레이어가 책임을 지키지 않으면 바로 문제가 드러났다. 예를 들어, 서비스 레이어에서 처리해야 할 로직을 컨트롤러에서 처리하거나, 레포지토리에서 비즈니스 로직을 다루기 시작하면 코드가 빠르게 복잡해졌고, 이후 수정할 때 영향 범위가 커지는 걸 직접 경험했다. 이 과정을 통해 구조는 형식이 아니라 변경에 대응하기 위한 장치라는 걸 이해하게 됐다. </br>

인증 로직을 다루면서도 단순 구현 이상의 고민이 이어졌다. JWT 기반 인증이 이미 구성되어 있었지만, 실제로는 토큰이 발급되고 검증되는 흐름, 만료 이후 재발급 과정, 그리고 클라이언트와의 상호작용까지 모두 연결해서 이해해야 했다. 특히 refresh 흐름을 제대로 이해하지 못했을 때는 왜 특정 요청에서 401이 발생하는지조차 명확히 설명하기 어려웠고, 로그를 따라가며 흐름을 추적하는 과정에서 코드를 아는 것과 흐름을 이해하는 것은 완전히 다른 영역이라는 걸 느꼈다. </br>

또 하나 어려웠던 부분은 모노레포 환경에서의 의존성 흐름이었다. 이미 분리된 패키지 구조를 그대로 사용해야 했기 때문에, 어떤 코드를 어디에 추가해야 하는지에 대한 판단이 중요했다. 처음에는 공통으로 쓸 것 같다는 이유로 패키지에 코드를 추가했다가, 오히려 특정 도메인에 강하게 의존하게 되는 문제를 겪으면서 공통화는 재사용 가능성보다 의존성 방향이 더 중요하다는 기준으로 스스로 세우게 됐다. </br>

이 프로젝트를 통해 가장 크게 달라진 점은, 더 이상 코드를 작성하는 데 집중하지 않게 됐다는 것이다. 대신 주어진 구조를 해석하고, 그 구조 안에서 일관성을 유지하는 것이 훨씬 중요하다는 걸 알게 됐다. 잘 설계된 구조 위에서는 빠르게 개발할 수 있지만, 그 구조를 이해하지 못하면 오히려 더 많은 시간을 쓰게 된다는 것도 함께 느꼈다. </br>

앞으로는 단순히 기능을 구현하는 수준을 넘어서, 이런 구조를 스스로 설계할 수 있는 단계까지 가는 것이 목표다. 이미 잘 만들어진 구조를 사용하는 경험을 했기 때문에, 다음에는 왜 이렇게 설계했는지를 설명할 수 있는 수준, 그리고 필요하다면 더 나은 방향으로 개선할 수 있는 수준까지 끌어올리는 게 다음 과제가 될 것 같다.
